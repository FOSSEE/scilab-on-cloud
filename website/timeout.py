...
signal : int : the signal to send to the child process at timeout
(default: SIGKILL) raisePrevSigalrmError : bool : if True, throw an
exception if there's a previous non-nop SIGALRM handler installed 
(default: True) childProcessGroup : int : if 0 (default), put child
process into its own process group if non-zero, put child process into 
the specified process group if None, inherit the parent's process group"""
...
"""
        ...
        if self.timeout is not None:
            ...
            if signal.getsignal(signal.SIGALRM) not in (None, signal.SIG_IGN,
                                                        signal.SIG_DFL):
            ...
            signal.signal(signal.SIGALRM,
                          lambda sig, frame: os.killpg(self.pgid,
                                                       self.timeoutSignal))
...
(default: SIGKILL)"""
        try:
            ...
            if e.errno == errno.ESRCH:
                # We end up here if the process group has already exited,
                # so it's safe to
                # ignore the error
                pass
            else:
...
if __name__ == "__main__":
    if len(sys.argv) <= 2:
        ...
        descrip = """Runs COMMAND for up to TIMEOUT seconds, sending COMMAND 
a SIGKILL if it attempts to run longer. Exits sooner if COMMAND does so, passing
 along COMMAND's exit code.
